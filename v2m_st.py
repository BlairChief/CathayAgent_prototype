...
if 'initialized' not in st.session_state:
    ...
    try:
        ...
        logger.info("所有agents初始化成功")
        logger.info("初始化聊天記錄")
        logger.info("程式啟動...")
    except Exception as e:
        logger.error(f"Agent初始化錯誤: {str(e)}")
...
def process_input(text_prompt):
    ...
    try:
        ...
        logger.info(f"收到新的輸入: {text_prompt[:50]}...")
        v2m_team = initv2m()
        response_text = ""
...
def handle_transcription(transcription):
    if transcription:
        logger.info("開始處理轉譯結果")
        try:
            # 處理並生成回應
            process_input(transcription)

            logger.info("語音輸入處理完成")
